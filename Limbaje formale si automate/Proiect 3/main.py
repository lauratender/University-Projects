...
#voi citi datele din fisierul "proiect3.in" astfel: 'litera simbol neterminal' 'spatiu' 'simbol terminal' 'litera simbol neterminal'
#voi afisa rezultatul in fisierul "proiect3.out"
...
# acest proiect este un update la proiectul al al doilea, am adaugat transformarea dintr-o gramatica regulata intr-un lambda NFA
...
def gramatica_regulata_lambdaNFA():
    # voi retine datele intr-un dictionar de forma simbol neterminal : lista ce contine perechi simbol terminal, simbol
    # neterminal care pleaca din cheie
    # starea initiala a fi mereu S si va primi numarul 0
    # voi avea o unica stare finala ce inital va primi numarul 26 pentru a nu coincide cu cifrele asociate literelor

    dictLitere = { 'S': 0 }     # dictionarul in care retin ce numar de nod ii va corespunde fiecarui simbol neterminal
    dict = {}   #dictionarul in care retin datele citite
    l = f.readline()
    M = 0   # numarul de muchii
    idNod = 0  #numarul nodului ce va fi asociat unei litere, acesta metoda face ca nodurile sa aiba numere consecutive indiferent
               # cum sunt alese litere simbol neterminal
    while l:

        M+=1
        l = l.split()

        # daca un nod nu are asociat un id ii asociez unul apoi lucrez cu acest id in dictionar

        # nodPlecare
        if l[0] in dictLitere.keys():
            nodPlecare = dictLitere[l[0]]
        else:
            idNod+=1
            nodPlecare =  idNod
            dictLitere.update ({l[0]: idNod })

        #nodDestinatie
        if len(l[1]) == 2:
            if l[1][1] in dictLitere.keys():
                nodDestinatie = dictLitere[l[1][1]]
            else:
                idNod += 1
                nodDestinatie = idNod
                dictLitere.update({l[1][1]: idNod})
        else:
            nodDestinatie = 26

        # adaug datele citite in dictionar
        if nodPlecare in dict.keys():
            dict[nodPlecare].append([l[1][0], nodDestinatie])
        else:
            dict.update({nodPlecare: [[l[1][0], nodDestinatie]]})
        l = f.readline()

    automat = lambdaNFA(len(dict) + 1, M, 1, 0)

    #starea finala va deveni din 26 automat.nrNoduri - 1
    automat.end[automat.nrNoduri - 1] = 1

    #adaug muchiile
    for key in dict.keys():
        for pereche in dict[key]:
            if pereche[1] == 26:
                automat.adaugaMuchie(key, automat.nrNoduri - 1, pereche[0])
            else:
                automat.adaugaMuchie(key, pereche[1], pereche[0])
    return automat
...
def main():
    ...
    # functie pentru proiectul 3  -------    gramatica regulata -> lambda NFA

    automat = gramatica_regulata_lambdaNFA()
    #afisare(automat)
...
f = open("proiect3.in", "r")
g = open("proiect3.out", "w")
